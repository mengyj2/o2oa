...
//document.querySelector("head").appendHTML('<meta http-equiv="Content-Security-Policy" content="upgrade-insecure-requests" />')
	...
	loadApplication: function(callback){ 
		this.maxSize(function () {
			...
			MWF.xDesktop.requireApp("CRM", "BaiduMap", function(){
				...
				var apiPath = ""; /* O2OA-NOMAP-PATCH: 原 ak=Qac4WmBvHXiC87z3HjtRrbotCE3sC9Zg 已失效(error 201) */
				if( false /* O2OA-NOMAP-PATCH: 停用百度地图脚本加载 */ && !window.BDMapApiLoaded ){
	...
	createTopContent:function(){
		...
		this.tmpImg = new Element("img.topLeftLiImg",{
			"styles":this.css.topLeftLiImg,
			"src":"../o2_core/o2/xDesktop/$Default/blue/icons/logout.png"
		}).inject(topLeftLi);

		this.tmpImg.addEvents({
			"click":function(){
				this.logout();
			}.bind(this)
		});
		/*this.loginConentDiv = new Element("ul.el-dropdown-type",{
			"styles": {"display": "none","width":"75px","top":"50px","right":"20px"},
			"id" :"inputLogin"
		}).inject(this.topContentDiv);
		this.headLoginLi = new Element("li.el-dropdown-menu__item",{
			"styles":{"font-size":"16px"},
			"text" :"注销"
		}).inject(this.loginConentDiv);*/

	},
	isAdmin: function(){
		return   MWF.AC.isCRMManager() || MWF.AC.isAdministrator()
	},
	logout:function(){
