...
MWF.xApplication.CRM.AddressExplorer.BaiduMap = new Class({
    ...
    loadResource: function (callback) {
        ...
        var apiPath = "";  /* O2OA-NOMAP-PATCH: 原 AK 已失效(error 201)，不再加载百度脚本 */
    ...
    /* O2OA-NOMAP-PATCH guard: 无 BMap 时优雅降级，不抛 ReferenceError */
    createMap: function( position ){
        if (typeof BMap === 'undefined' || !BMap || !BMap.Map) {
            try { this.mapNode.innerHTML = '<div style="padding:16px;color:#888;font-size:13px;">地图功能不可用（内置百度地图 AK 已失效），客户/联系人/线索等其它功能均正常。</div>'; } catch (e) {}
            return;
        }
        var point = null;
