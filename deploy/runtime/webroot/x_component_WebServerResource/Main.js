...
MWF.xDesktop.requireApp("WebServerResource", "AttachmentController", null, false);
MWF.require("MWF.widget.Tree", null, false);
MWF.xApplication.WebServerResource.Main = new Class({
	...
	options: {
		...
		"width": "1000",
		"height": "600",
		"title": MWF.xApplication.WebServerResource.LP.title
	},
	onQueryLoad: function(){
		this.lp = MWF.xApplication.WebServerResource.LP;
        this.restActions = MWF.Actions.load("x_program_center");
	},
	loadApplication: function(callback){
		...
		MWF.getJSON("../x_component_WebServerResource/$Main/icon.json", function(json){
			this.icons = json;
                ...
                if (this.status){
                    if (this.status.node){
                        ...
                        for (var i=0; i<this.editorTree.children.length; i++){
                            if (this.editorTree.children[i].data.fileName == this.status.node){
...
//				var y = e.event.y;
    ...
    openAttachment: function(e, node, attachment){
        var href = window.location.protocol + "//" + window.location.host + attachment[0].data.url;
        window.open(href);

    },
    downloadAttachment: function(e, node, attachments){
        attachments.each(function(attachment){
            var href = window.location.protocol + "//" + window.location.host + attachment.data.url;
            window.open(href);
        }.bind(this));
    },
	...
	renameFileFolder: function(){
		...
		var inputNode = new Element("input", {"type": "text", "styles": this.css.createFolderInputNode, "value": item.data.fileName}).inject(inputAreaNode);
		...
		okButton.addEvent("click", function(){
			if (inputNode.get("value")){
                item.data.fileName = inputNode.get("value");
	...
	deleteFileFolder: function(e, attachments){
		if (attachments.length){
            ...
            if (attachments.length==1){
                ...
                content = content+"（"+attachments[0].data.fileName+"）";
            }
    ...
    uploadAttachment: function(e, node){
        ...
        filePath = (this.currentFolder && this.currentFolder.data) ? this.currentFolder.data.filePath : "/";

        var action = MWF.Actions.get("x_program_center");
        action.action.actions.dispatchResource = {
            "uri": "/jaxrs/module/dispatch/resource/as/new/{asNew}",
            "method": "POST",
            "enctype": "formData"
        }

        this.controller.doUploadAttachment({
            "filePath": filePath || "/"
        }, action.action, "dispatchResource" ,{
            "asNew" : true,
        },function(){
            if (this.currentFolder){
                if(filePath === "") this.currentFolder.data = null;
                this.updatePathNode(this.currentFolder);

            }else{
                this.updatePathNode(this.topTreeNode);
            }
        }.bind(this));
    },
...
//		}.bind(this));
    ...
    createEditorTreeNode: function(json){
        json.data.each(function(data){
            var name = MWF.name.cn(data.fileName);
            var nodeData = {
                "expand": false,
...
//			this.loadFolderTree();
    ...
    loadEditorFile: function(treeNode){
        var person = treeNode.data.fileName;
	...
	loadFileContentAreaNode: function(){
        ...
        this.controller = new MWF.xApplication.WebServerResource.AttachmentController(this.attachmentContentNode, this, {
            "resize": false,
            "isSizeChange": false,
        })
	...
	createTreeNode: function(folderList, topNode){

        topNode.getChildrenNode().empty();
        ...
        folderList.each(function(data){
            ...
            data.lastUpdateTime = data.lastModifyTime;
	...
	loadFolderTree: function(node, callback){
		...
		if (!node.loaded){
			...
			if (node.data){
                ...
                this.restActions.SysResourceAction.listResource(encodeURIComponent(node.data.filePath), function( json ){
                    var folderList = json.data.folders;
                    var fileList = json.data.files;
                    this.createTreeNode(folderList,node);
                    node.setOperateIcon();
                ...
                this.restActions.SysResourceAction.listResource("(0)", function( json ){
                    var folderList = json.data.folders;
                    var fileList = json.data.files;
                    this.createTreeNode(folderList,node);
                    node.setOperateIcon();
...
//		this.fileContentAreaNode.setStyle("min-height", height);
	...
	setPathNode: function(treeNode){
		...
		paths.each(function(node, index){
			...
			var pathNode = new Element("div",{
				...
				"text": (node.data) ? node.data.fileName : "root",
				"events": {
					"mouseover": function(){
	...
	loadSub: function(treeNode){
        ...
        if(fileList){
            ...
            fileList.each(function(file){
                ...
                file.lastUpdateTime = file.lastModifyTime;
                file.extension = name.substring(name.lastIndexOf(".")+1 ,name.length);
                this.controller.addAttachment(file);
    ...
    updatePathNode : function (node){
        node.loaded = false;
        this.loadFolderTree(node,function (){
            node.clickNode();
        });
    },
...
MWF.xApplication.WebServerResource.Attachment = new Class({
	initialize: function(data, file){
		this.data = data;
		this.file = file;
		this.type = "file";

		this.extension = this.data.fileName.substr(this.data.fileName.lastIndexOf(".")+1, this.data.fileName.length);

		this.load();
	},
	getIcon: function(){
		var iconName = this.file.icons[this.extension] || this.file.icons.unknow;
		return "../x_component_WebServerResource/$Main/default/file/"+iconName;
	},
	load: function(){
...
MWF.xApplication.WebServerResource.Folder = new Class({
	initialize: function(data, file){
		this.data = data;
		this.file = file;
		this.type = "folder";
	...
	getIcon: function(){
		return "../x_component_WebServerResource/$Main/default/file/folder.png";
	},
	load: function(){
		this.node = new Element("div", {"styles": this.file.css.attachmentNode});
	//	this.node.addEvents();
